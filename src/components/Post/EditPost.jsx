...
export default function EditPost() {
    ...
    useEffect(() => {
        if (editPostObjHook) {
            setCaption(editPostObjHook.postCaption);
            setPrice(editPostObjHook.postPrice);
            setImagePreview(editPostObjHook.postImage);
            setproductName(editPostObjHook.productName)
        }
    }, [editPostObjHook]);
    ...
    const updatePost = async (e) => {
        ...
        formData.append('postid', editPostObjHook.postId);
        ...
        try {
            ...
            const res = await put(`/post`, formData, {
                headers: {
    ...
    return (
        <React.Fragment>
            {postEditHook && (
                <>
                    <div
                        className="scale-125 fixed top-0 left-0 right-0 bottom-0"
                        onClick={() => setpostEditHook(false)}
                        style={{ background: "rgb(189, 189, 189, 0.9)" }}
                    ></div>
                    <form action="" onSubmit={updatePost}>
                        <div
                            className="flex flex-col justify-center gap-y-2 fixed z-10 left-[50%] w-[90vw] top-[50%] transition duration-300 sm:w-[50vw] bg-[white] rounded-lg shadow-lg py-4 px-4"
                            style={{ transform: "translate(-50% , -50%)" }}
                        >
                            <div className="flex justify-between items-center">
                                <p></p>
                                <h1 className="text-xl font-bold text-center">Edit Post</h1>
                                <i
                                    className="fa-solid fa-xmark text-[20px] hover:bg-gray-200 p-2 rounded-full cursor-pointer hover:transition-all hover:duration-500"
                                    onClick={() => setpostEditHook(false)}
                                ></i>
                            </div>
                            ...
                            <div className="flex items-center justify-center">
                                <label
                                    htmlFor="file"
                                    className="flex flex-col items-center justify-center w-[300px] h-[200px] gap-5 cursor-pointer border-2 border-dashed border-gray-300 p-6 rounded-lg shadow-[0px_48px_35px_-48px_rgba(232,232,232,0.6)]"
                                >
                                    <div className="flex items-center justify-center">
                                        <svg
                                            viewBox="0 0 24 24"
            ...
            {errorModal && <ErrorModal error="error updating post" errormsg={popupMessage} />}
        </React.Fragment>
    );
}
